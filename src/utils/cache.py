...
class ChatCache:
    ...
    def save_auth(self, pin_hash, pin_salt):
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('''
            INSERT INTO auth (id, pin_hash, pin_salt, failed_attempts, is_configured)
            VALUES (1, ?, ?, 0, 1)
    ...
    def get_auth(self):
        ...
        cursor = conn.cursor()
        cursor.execute('''
            SELECT pin_hash, pin_salt, failed_attempts, is_configured
            FROM auth
            WHERE id = 1
        ''')
